...
class MyInput implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    // Emit marker immediately before the reverse-video cursor cell
...
Cursor presentation is owned entirely by the TUI: components always emit `CURSOR_MARKER` plus a reverse-video cursor cell, and the TUI decides which cursor the user sees. The cursor remains hidden by default, which keeps the painted cursor rendering while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Enable it with the renderer constructor's `showHardwareCursor` argument or `setShowHardwareCursor(true)` so the terminal owns cursor shape, blink, and hollow-on-blur. The `Editor` and `Input` built-in components already implement this interface.
