...
describe("hardware cursor default", () => {
	// pi-tui stays config-agnostic: it reads no environment, so the caller owns the
	// setting. Pi resolves PI_HARDWARE_CURSOR in SettingsManager.getShowHardwareCursor()
	// and injects the result here.
	it("is off unless the caller enables it", () => {
		assert.equal(new TuiMainScreen(new VirtualTerminal()).getShowHardwareCursor(), false);
	});

	it("honors an explicit constructor override", () => {
